...
# %matplotlib inline   # 在Jupyter-notebook中可以取消注释
        ...
        for h in range(n_H):  # loop over vertical axis of the output volume
            for w in range(n_W):  # loop over horizontal axis of the output volume
                for c in range(n_C):  # loop over channels (= #filters) of the output volume
                    ...
                    # Find the corners of the current "slice" (≈4 lines) 问题出现在这儿
                    vert_start = h * stride
                    vert_end = h * stride + f
...
def pool_forward(A_prev, hparameters, mode="max"):
    ...
    ### START CODE HERE ###
    for i in range(np.shape(A_prev)[0]):  # loop over the training examples
...
def pool_backward(dA, cache, mode="max"):
    ...
    for i in range(m):  # loop over the training examples
        ...
        for h in range(n_H_prev):  # loop on the vertical axis
            for w in range(n_W_prev):  # loop on the horizontal axis
                for c in range(n_C_prev):  # loop over the channels (depth)
